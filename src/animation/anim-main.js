...
function onWorkerMessage(e) {
    ...
    switch (msg.type) {
        case 'ready':
            if (workerInitTimer) { clearTimeout(workerInitTimer); workerInitTimer = null; }
            workerReady = true;
            // Send initial animation state and config to the worker
...
/* Palette selector removed — color is now controlled via hue/spectrum/chroma sliders */
...
const animCfg = { sparkle: savedAnimConfig.sparkle ?? 1, drift: savedAnimConfig.drift ?? 1, wobble: savedAnimConfig.wobble ?? 1 };
...
function syncAnimConfigVisibility() {
...
document.addEventListener('localechange', () => {
    document.title = t('page.animation');
    scheduleRender();
});
